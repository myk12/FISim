    ...
    static TypeId tid =
        TypeId("ns3::MpTcpBulkSendApplication")
            ...
            .AddAttribute("Protocol",
                          ...
                          TypeIdValue(MpTcpSocketFactory::GetTypeId()),
...
{
    NS_LOG_FUNCTION(this);
    // NS_LOG_UNCOND(Simulator::Now().GetSeconds() << " StartApplication -> Node-FlowId: {" <<
    // GetNode()->GetId() <<"-" << m_flowId<< "} MaxBytes: " << m_maxBytes << " F-Type: " <<
    // m_flowType << " S-Time: " << m_simTime);
    //  Create the socket if not already
    if (m_socket == nullptr)
    {
        NS_LOG_DEBUG("MPTCP bulk send app Creating Scoket.");
        // m_socket = CreateObject<MpTcpSocketBase>(GetNode()); //m_socket = Socket::CreateSocket
        // (GetNode (), m_tid); m_socket = DynamicCast<MpTcpSocketBase>(Socket::CreateSocket
        // (GetNode (), m_tid));
        m_socket = DynamicCast<MpTcpSocketBase>(
            Socket::CreateSocket(GetNode(), MpTcpSocketFactory::GetTypeId()));
        NS_ASSERT(m_socket);
        m_socket->Bind();
...
MpTcpBulkSendApplication::HandlePeerClose(Ptr<Socket> socket)
{
    ...
    NS_LOG_UNCOND("*** [" << m_node->GetId()
                          << "] HandlePeerError is called -> connection is false");
    m_connected = false;

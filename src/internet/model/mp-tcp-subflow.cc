      ...
      lastMeasuredRtt(Seconds(0.0))
{
    ...
    highestAck = 0;
    rtt = CreateObject<RttMeanDeviation2>();
    rtt->Gain(0.1);
    Time estimate;
...
MpTcpSubFlow::CwndTracer(uint32_t oldval, uint32_t newval)
{
    ...
    rttTracer.push_back(
        std::make_pair(Simulator::Now().GetSeconds(), rtt->GetCurrentEstimate().GetMilliSeconds()));
    rtoTracer.push_back(
        std::make_pair(Simulator::Now().GetSeconds(), rtt->RetransmitTimeout().GetMilliSeconds()));
}

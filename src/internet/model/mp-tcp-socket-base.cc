...
std::string outputfilename = "MPTCP_GNUPLOT.pdf";
...
{ // Any packet without SYN and MP_CAPABLE is not being processed!
    ...
    const TcpHeader::TcpOptionList options = mptcpHeader.GetOptionList();
    uint8_t flags = mptcpHeader.GetFlags();
    bool hasSyn = flags & TcpHeader::SYN;

    if (options.size() <= 0)
    {
        NS_LOG_DEBUG("No MPTCP options found.");
    }

    for (it = options.begin(); it != options.end(); ++it)
    {
        const Ptr<const TcpOption> option = (*it);
        NS_LOG_DEBUG("Reading options " << option);
        if ((option->GetKind() == TcpOption::MPC) && hasSyn && (mpRecvState == MP_NONE))
        { // SYN+ACK would be send later on by ProcessSynRcvd(...)
            Ptr<const TcpOptionMpCapable> opt = DynamicCast<const TcpOptionMpCapable>(option);
            mpRecvState = MP_MPC;
            mpEnabled = true;
            remoteToken = opt->GetSenderToken();
            NS_ASSERT_MSG(remoteToken != 0, "Error: Remote token is zero."); // Correct condition
            return true;
        }
        else
        {
...
MpTcpSocketBase::ReadOptions(uint8_t sFlowIdx, Ptr<Packet> pkt, const TcpHeader& mptcpHeader)
{
    ...
    for (it = options.begin(); it != options.end(); ++it)
    {
        ...
        else if ((option->GetKind() == TcpOption::JOIN) && hasSyn)
        {
            Ptr<const TcpOptionJoinConn> opt = DynamicCast<const TcpOptionJoinConn>(option);
            if ((mpSendState == MP_ADDR) && (localToken == opt->GetReceiverToken()))
            { // SYN+ACK would be send later on by ProcessSynRcvd(...)
                // Join option is sent over the path (couple of addresses) not already in use
                NS_LOG_UNCOND("Server receive new subflow!");
            }
        }
        else if ((option->GetKind() == TcpOption::ADDR) && (mpRecvState == MP_MPC))
        {
            // Receiver store sender's addresses information and send back its addresses.
            // If there are several addresses to advertise then multiple TcpOption::ADDR would be
            // attached to the TCP Options.
            Ptr<const TcpOptionAddAddr> opt = DynamicCast<const TcpOptionAddAddr>(option);
            MpTcpAddressInfo* addrInfo = new MpTcpAddressInfo();
            addrInfo->addrID = opt->GetAddrID();
            addrInfo->ipv4Addr = opt->GetIpv4Addr();
            remoteAddrs.insert(remoteAddrs.end(), addrInfo);
            TxAddr = true;
        }
        else if (option->GetKind() == TcpOption::DSN)
        { // not implemented yet
            NS_LOG_LOGIC(this << " ReadOption-> TcpOption::DSN -> we'll deal with it later on");
        }
        else if (hasSyn)
                                    ...
                                    const TcpHeader& mptcpHeader)
{
    ...
    { // Received RST or the TCP flags is invalid, in either case, terminate this socket
        ...
        { // this must be an invalid flag, send reset
            ...
            std::cout << Simulator::Now().GetSeconds() << " [" << m_node->GetId() << "] ("
                      << (int)sFlowIdx << ") {" << flowId << "} SendRST(ProcessEstablished)"
                      << std::endl;
            SendRST(sFlowIdx);
        }
                              ...
                              const Address& toAddress)
{
    ...
    sFlow->state = MpTcpSubFlow::TcpStates_t::SYN_RCVD;
...
MpTcpSocketBase::ProcessSynSent(uint8_t sFlowIdx, Ptr<Packet> packet, const TcpHeader& mptcpHeader)
{
    ...
    { // Handshake completed for sender... Send final ACK
        ...
        NS_LOG_INFO("(" << sFlow->routeId << ") " << TcpStateName[sFlow->state]
                        << " -> ESTABLISHED");
        sFlow->state = MpTcpSubFlow::TcpStates_t::ESTABLISHED;
    ...
    { // Other in-sequence input
        ...
        { // When (1) rx of FIN+ACK; (2) rx of FIN; (3) rx of bad flags
            ...
            NS_LOG_UNCOND(Simulator::Now().GetSeconds()
                          ...
                          << ") Bad FtcpFlag received - SendRST");
            std::cout << Simulator::Now().GetSeconds() << " [" << m_node->GetId() << "] ("
                      << (int)sFlowIdx << ") {" << flowId << "} SendRST(ProcessSynSent)"
                      << std::endl;
            SendRST(sFlowIdx);
        }
        CloseAndNotifyAllSubflows();
...
void
MpTcpSocketBase::ProcessSynRcvd(uint8_t sFlowIdx,
                                Ptr<Packet> packet,
                                const TcpHeader& mptcpHeader,
                                const Address& fromAddress,
                                const Address& toAddress)
{
    ...
    { // handshake is completed nicely in the receiver.
        ...
        sFlow->state = MpTcpSubFlow::TcpStates_t::ESTABLISHED; // Subflow state is ESTABLISHED
    ...
    { // Other in-sequence input
        ...
        { // When (1) rx of SYN+ACK; (2) rx of FIN; (3) rx of bad flags
            ...
            NS_LOG_UNCOND(Simulator::Now().GetSeconds()
                          ...
                          << ") Bad FtcpFlag received - SendRST");
            std::cout << Simulator::Now().GetSeconds() << " [" << m_node->GetId() << "] ("
                      << (int)sFlowIdx << ") {" << flowId << "} SendRST(ProcessSynRcvd)"
                      << std::endl;
...
MpTcpSocketBase::ProcessWait(uint8_t sFlowIdx, Ptr<Packet> packet, const TcpHeader& mptcpHeader)
{
    ...
    { // This is a RST or bad flags
        ...
        if (tcpflags != TcpHeader::RST)
        {
            NS_LOG_LOGIC("Illegal flag " << tcpflags << " received. Reset packet is sent.");
            std::cout << Simulator::Now().GetSeconds() << " [" << m_node->GetId() << "] ("
                      << (int)sFlowIdx << ") {" << flowId << "} SendRST(ProcessWait)" << std::endl;
            SendRST(sFlowIdx);
        }
        CloseAndNotifyAllSubflows();
        return;
    ...
    if ((sFlow->state == MpTcpSubFlow::TcpStates_t::FIN_WAIT_1 ||
         sFlow->state == MpTcpSubFlow::TcpStates_t::FIN_WAIT_2) &&
        sFlow->Finished())
    {
        if (sFlow->state == MpTcpSubFlow::TcpStates_t::FIN_WAIT_1)
        {
...
MpTcpSocketBase::TimeWait(uint8_t sFlowIdx)
{
    ...
    NS_LOG_INFO("(" << (int)sFlow->routeId << ") " << TcpStateName[sFlow->state]
                    << " -> TIME_WAIT {TimeWait}");
    sFlow->state = MpTcpSubFlow::TcpStates_t::TIME_WAIT;
...
MpTcpSocketBase::CancelAllTimers(uint8_t sFlowIdx)
{
    ...
    NS_LOG_LOGIC("(" << (int)sFlow->routeId << ")"
                     << "CancelAllTimers");
}
...
MpTcpSocketBase::ProcessLastAck(uint8_t sFlowIdx, Ptr<Packet> packet, const TcpHeader& mptcpHeader)
{
    ...
    { // Received a SYN or SYN+ACK or bad flags
        ...
        std::cout << Simulator::Now().GetSeconds() << " [" << m_node->GetId() << "] ("
                  << (int)sFlowIdx << ") {" << flowId << "} SendRST(ProcessLastAck)" << std::endl;
        SendRST(sFlowIdx);
        CloseAndNotifyAllSubflows();
    }
}
...
MpTcpSocketBase::ReceivedData(uint8_t sFlowIdx, Ptr<Packet> p, const TcpHeader& mptcpHeader)
{
    ...
    {
        const Ptr<const TcpOption> option = (*it);
        if (option->GetKind() == TcpOption::DSN)
        {
            Ptr<const TcpOptionDataSeqMapping> opt =
                DynamicCast<const TcpOptionDataSeqMapping>(option);
            // NS_ASSERT(optDSN->subflowSeqNumber == Seq);
            uint64_t dataSeqNum = opt->GetDataSeqNumber();
            uint16_t dataLevelLen = opt->GetDataLevelLength();
            uint32_t subflowSeqNum = opt->GetSubflowSeqNumber();

            if (subflowSeqNum == sFlow->RxSeqNumber)
            { /* Received packet is in-sequence at sub-flow level. Now check connection level? */
                if (dataSeqNum == nextRxSequence)
                { /** Received packet is in-sequence at connection level but in-order at sub-flow
                     level **/
                    uint32_t amountRead = recvingBuffer.ReadPacket(p, opt->GetDataLevelLength());
                    if (amountRead == 0)
                    {
                        NS_FATAL_ERROR("I don't see any reason to trigger this condition, at least "
                                       "in current implementation");
                        return;
                    }
                    NS_ASSERT(amountRead == dataLevelLen && dataLevelLen == p->GetSize());
                ...
                else if (dataSeqNum > nextRxSequence) // there is a gap in dataSeqNumber
                { /** Received packet is out of sequence at connection level but in-order at
                     sub-flow level **/
                    ...
                    // For allowing sub-flow to progress, RxSeqNb should be advanced even though
                    // packet is not in-order of connection level.
                    if (stored)
                    {
                        NS_ASSERT(subflowSeqNum == sFlow->RxSeqNumber);
                        sFlow->RxSeqNumber += dataLevelLen;
                        sFlow->highestAck = std::max(sFlow->highestAck,
                                                     (mptcpHeader.GetAckNumber()).GetValue() - 1);
                    }
                    // We need to send ACK here to indicate that a packet leaves a network and
                    // signaling to sender that which sequence number is expected to receive at
                    // sub-flow level.
                    SendEmptyPacket(sFlowIdx, TcpHeader::ACK);
                }
                else
                { /** Received packet is duplicated in connection level! */
                    NS_ASSERT(dataSeqNum < nextRxSequence);
                    NS_FATAL_ERROR("This functionality is not yet implemented!");
                    NS_LOG_WARN(this << "Duplicated segment received at connection level so it "
                                        "should be rejected!");
                    SendEmptyPacket(sFlowIdx, TcpHeader::ACK);
                }
            }
            else if (subflowSeqNum > sFlow->RxSeqNumber)
            { /* Received packet is out of order at sub-flow level */
                // This condition might occurs when a packet get drop...Does this condition mean
                // that packet should be out of order at connection level? YES
                NS_ASSERT(dataSeqNum > nextRxSequence);
                ...
                SendEmptyPacket(sFlowIdx,
                                TcpHeader::ACK); // We need to send ACK regardless of whether
                                                 // segment has already stored in unOrdered or not!
            }
            else if (subflowSeqNum < sFlow->RxSeqNumber)
            { /* Received packet is duplicated at sub-flow level. It should be rejected!*/
                NS_LOG_INFO("Data received is duplicated in Subflow Layer so it has been rejected! "
                            "subflowSeq: "
                            << subflowSeqNum << " dataSeq: " << dataSeqNum);
                SendEmptyPacket(
                    sFlowIdx,
...
MpTcpSocketBase::ReceivedAck(uint8_t sFlowIdx, Ptr<Packet> packet, const TcpHeader& mptcpHeader)
{
    ...
    { // Case 3: New ACK, reset m_dupAckCount and update m_txBuffer (DSNMapping List)
        ...
        NewAckNewReno(sFlowIdx, mptcpHeader);
...
MpTcpSocketBase::SendDataPacket(uint8_t sFlowIdx, uint32_t size, bool withAck)
{
    ...
    if (sFlow->maxSeqNb > sFlow->TxSeqNumber - 1)
    {
        ...
        for (std::list<DSNMapping*>::iterator it = sFlow->mapDSN.begin();
    ...
    if (p == nullptr && ptrDSN == nullptr)
    {
        ...
        { // TODO I guess we should not return from here - What do we do then kill ourself?
            ...
            NS_ASSERT_MSG(p, "No data is available in SendingBuffer to create a pkt from it!");
            return 0;
        }
    }
    ...
    { // if packet is made from sendingBuffer, then we use nextTxSequence to OptDSN
        AddOptionDSN(header, nextTxSequence, packetSize, sFlow->TxSeqNumber);
    }
    else
    { // if packet is made from subflow's Buffer (already sent packets), that packet's dataSeqNumber
      // should be added here!
        AddOptionDSN(header, ptrDSN->dataSeqNumber, (uint16_t)packetSize, sFlow->TxSeqNumber);
...
MpTcpSocketBase::SendAllSubflowsFIN(void)
{
    ...
    if (sendingBuffer.PendingData() == 0)
    {
        ...
        for (SubflowCI = subflows.begin(); SubflowCI != subflows.end(); ++SubflowCI)
        {
            ...
            {
                NS_LOG_DEBUG(this << " " << Simulator::Now().GetSeconds()
...
MpTcpSocketBase::DoRetransmit(uint8_t sFlowIdx)
{
    ...
    if (ptrDSN == 0)
    {
        NS_LOG_INFO("Retransmit -> no Unacked data !! mapDSN size is "
                    << sFlow->mapDSN.size() << " max Ack seq number " << sFlow->highestAck << " ("
                    << (int)sFlowIdx << ")");
        NS_ASSERT(3 != 3);
        return;
    }
...
MpTcpSocketBase::Connect(Ipv4Address servAddr, uint16_t servPort)
{
    ...
    // m_segmentSize << " segmentSize: " << segmentSize << "SendingBufferSize: " <<
    // sendingBuffer.bufMaxSize);
    ...
    NS_LOG_INFO("SubFlow[" << (int)sFlow->routeId << "] status: " << TcpStateName[sFlow->state]
                           << " -> SYN_SENT");
    m_state = SYN_SENT;
    sFlow->state = MpTcpSubFlow::TcpStates_t::SYN_SENT; // Subflow state should be change first then
                                                        // SendEmptyPacket...
...
MpTcpSocketBase::SendPendingData(uint8_t sFlowIdx)
{
    ...
    if (sendingBuffer.Empty() && sFlowIdx < maxSubflows)
    {
        ...
        { // SendingBuffer is empty but subflowBuffer (mapDSN) is not. Also subflow is recovering
          // from timeOut.
...
MpTcpSocketBase::ReduceCWND(uint8_t sFlowIdx, DSNMapping* ptrDSN)
{
    ...
    case COUPLED_FULLY:
        d = (int)sFlow->cwnd.Get() - compute_total_window() / MPTCP_SOCKETBASE_B;
        if (d < 0)
            d = 0;
        sFlow->ssthresh = std::max(2 * mss, (uint32_t)d);
        sFlow->cwnd = sFlow->ssthresh + 3 * mss;
...
MpTcpSocketBase::getAckedSegment(uint8_t sFlowIdx, uint32_t ack)
{
    Ptr<MpTcpSubFlow> sFlow = subflows[sFlowIdx];
    DSNMapping* ptrDSN = 0;
    for (std::list<DSNMapping*>::iterator it = sFlow->mapDSN.begin(); it != sFlow->mapDSN.end();
         ++it)
    {
        DSNMapping* dsn = *it;
...
void
MpTcpSocketBase::NewAckNewReno(uint8_t sFlowIdx, const TcpHeader& mptcpHeader)
{
    NS_LOG_FUNCTION(this << (int)sFlowIdx);
    Ptr<MpTcpSubFlow> sFlow = subflows[sFlowIdx];
    ...
    { // Partial ACK, partial window deflation (RFC2582 sec.3 bullet #5 paragraph 3)
        ...
        NewACK(sFlowIdx,
               mptcpHeader); // update m_nextTxSequence and send new data if allowed by window
    ...
    if (!(sFlow->mapDSN.size() == 0 && sendingBuffer.Empty() &&
          sFlow->state == MpTcpSubFlow::TcpStates_t::FIN_WAIT_1))
        // MPTCP various congestion control algorithms...
        OpenCWND(sFlowIdx, ackedBytes);

    // Complete newAck processing
    NewACK(sFlowIdx, mptcpHeader); // update m_nextTxSequence and send new data if allowed by window
}

void
MpTcpSocketBase::NewACK(uint8_t sFlowIdx, const TcpHeader& mptcpHeader)
{
    Ptr<MpTcpSubFlow> sFlow = subflows[sFlowIdx];
    uint32_t ack = (mptcpHeader.GetAckNumber()).GetValue();
    NS_LOG_LOGIC(
...
MpTcpSocketBase::SendEmptyPacket(uint8_t sFlowIdx, uint8_t flags)
{
    ...
    if (hasSyn)
    {
        ...
        { // No more connection retries, give up
            std::cout << "[" << m_node->GetId() << "]{" << flowId << "}(" << flowType << ")"
                      << sFlow->cnCount << std::endl;
            NS_LOG_UNCOND(Simulator::Now().GetSeconds()
    ...
    if (((sFlow->state == MpTcpSubFlow::TcpStates_t::SYN_SENT) ||
         (sFlow->state == MpTcpSubFlow::TcpStates_t::SYN_RCVD && mpEnabled == true)) &&
        mpSendState == MP_NONE)
    {
        ...
        } while (m_tcp->IsTokenExist(localToken) || localToken == 0);
        ...
        m_tcp->InsertNewToken(localToken, m_endPoint);
                             ...
                             Ptr<Ipv4Interface> interface)
{
    ...
    if (subflows.size() == 0 && m_state == LISTEN)
    {
        ...
        {
            NS_LOG_LOGIC("Read Options Failed.");
            return;
        }
    ...
    case CLOSED:
        ...
        { // Since sFlow->m_endPoint is not configured yet, we cannot use SendRST here
            std::cout << Simulator::Now().GetSeconds() << " [" << m_node->GetId() << "] ("
                      << (int)sFlowIdx << ") {" << flowId << "} SendRST(DoForwardup)" << std::endl;
            TcpHeader h;
...
MpTcpSocketBase::InitiateSubflows()
{
    ...
    for (uint32_t i = 0; i < localAddrs.size(); i++)
        for (uint32_t j = i; j < remoteAddrs.size(); j++)
        {
            ...
            sFlow->state = MpTcpSubFlow::TcpStates_t::SYN_SENT;
            sFlow->cnTimeout = m_cnTimeout;
            sFlow->cnRetries = m_synRetries;
            sFlow->cnCount = sFlow->cnRetries;
            sFlow->m_endPoint = m_tcp->Allocate(GetBoundNetDevice(),
                                                sFlow->sAddr,
                                                sFlow->sPort,
                                                sFlow->dAddr,
...
MpTcpSocketBase::ReadUnOrderedData()
{
    ...
    while (next != unOrdered.end())
    {
        ...
        if ((ptrDSN->dataSeqNumber <=
             nextRxSequence) /*&& (ptrDSN->subflowSeqNumber == sFlow->RxSeqNumber)*/)
...
MpTcpSocketBase::GeneratePktCount()
{
    ...
    dataSetPKt.SetStyle(Gnuplot2dDataset::LINES);
    dataSetPKt.SetTitle("");
...
MpTcpSocketBase::DoPeerClose(uint8_t sFlowIdx)
{
    NS_LOG_FUNCTION((int)sFlowIdx);
    Ptr<MpTcpSubFlow> sFlow = subflows[sFlowIdx];
    NS_ASSERT(sFlow->state == MpTcpSubFlow::TcpStates_t::ESTABLISHED ||
              sFlow->state == MpTcpSubFlow::TcpStates_t::SYN_RCVD);
    /*
    ...
    if (sFlow->state == MpTcpSubFlow::TcpStates_t::LAST_ACK)
    {
        ...
        sFlow->m_lastAckEvent = Simulator::Schedule(sFlow->rtt->RetransmitTimeout(),
...
MpTcpSocketBase::CloseAndNotify(uint8_t sFlowIdx)
{
    ...
    sFlow->state = MpTcpSubFlow::TcpStates_t::CLOSED; // Can we remove closed subflow from subflow
                                                      // container????
...
MpTcpSocketBase::DoClose(uint8_t sFlowIdx)
{
    ...
    case CLOSE_WAIT:
        ...
        sFlow->state = MpTcpSubFlow::TcpStates_t::LAST_ACK;
        break;
    case SYN_SENT:
    case CLOSING:
        ...
        std::cout << Simulator::Now().GetSeconds() << " [" << m_node->GetId() << "]("
                  ...
                  << TcpStateName[sFlow->state] << " <-- SendRST(DoCLOSE)" << std::endl;
        SendRST(sFlowIdx);
        CloseAndNotifyAllSubflows();
        break;
    case LISTEN:
...
MpTcpSocketBase::LookupSubflow(Ipv4Address src, uint32_t srcPort, Ipv4Address dst, uint32_t dstPort)
{
    ...
    sFlow->MSS = segmentSize;
    sFlow->cwnd = sFlow->MSS;
    sFlow->state = MpTcpSubFlow::TcpStates_t::LISTEN;
    sFlow->cnTimeout = m_cnTimeout;
    sFlow->cnRetries = m_synRetries;
    sFlow->cnCount = sFlow->cnRetries;
    sFlow->m_endPoint = m_tcp->Allocate(GetBoundNetDevice(),
...
MpTcpSocketBase::compute_a_scaled()
{
    ...
    return (uint32_t)(MPTCP_SOCKETBASE_A_SCALE * (uint64_t)cwndSum * t / sum_denominator /
                      sum_denominator);
}
...
MpTcpSocketBase::OpenCWND(uint8_t sFlowIdx, uint32_t ackedBytes)
{
    ...
    else
    {
        ...
        case COUPLED_INC:
            ...
            tmp = tmp2 / MPTCP_SOCKETBASE_A_SCALE;
            ...
            if (rand() % MPTCP_SOCKETBASE_A_SCALE < tmp2 % MPTCP_SOCKETBASE_A_SCALE)
                tmp++;
        ...
        case COUPLED_FULLY:
            total_cwnd = compute_total_window();
            tt = (int)(ackedBytes * mss * MPTCP_SOCKETBASE_A);
            tmp = tt / total_cwnd;
            if (tmp > tcp_inc)
                tmp = tcp_inc;
            sFlow->cwnd = cwnd + tmp;
...
{
    assert(client);
    if ((subflows[0]->state >= MpTcpSubFlow::TcpStates_t::FIN_WAIT_1 ||
         subflows[0]->state == MpTcpSubFlow::TcpStates_t::CLOSED) &&
        flowCompletionTime)
    {
        int dataLeft = 0;
        int pktCount = 0;
        ...
        if (pktCount == 0)
        {
            std::cerr << "[" << m_node->GetId() << ":" << flowId << " -> pktCount is zero!"
                      << std::endl;
        }
...
//*******************************************************************************
bool
MpTcpSocketBase::AddOptionDSN(TcpHeader& header,
                              uint64_t dSeqNum,
                              uint16_t dLevelLen,
                              uint32_t subflowSeqNum)
{
    NS_LOG_FUNCTION(this);
    Ptr<TcpOptionDataSeqMapping> option = CreateObject<TcpOptionDataSeqMapping>();
    option->SetDataSeqNumber(dSeqNum);
    option->SetDataLevelLength(dLevelLen);
    option->SetSubflowSeqNumber(subflowSeqNum);
...
bool
MpTcpSocketBase::AddOptionMPC(TcpHeader& header, uint32_t TxToken)
{
    NS_LOG_FUNCTION(this);
    Ptr<TcpOptionMpCapable> option = CreateObject<TcpOptionMpCapable>();
    option->SetSenderToken(TxToken);
...
bool
MpTcpSocketBase::AddOptionJOIN(TcpHeader& header, uint32_t RxToken, uint8_t addrID)
{
    NS_LOG_FUNCTION(this);
    Ptr<TcpOptionJoinConn> option = CreateObject<TcpOptionJoinConn>();
    option->SetReceiverToken(RxToken);
    option->SetAddrID(addrID);
...
bool
MpTcpSocketBase::AddOptionADDR(TcpHeader& header, uint8_t addrID, Ipv4Address ipv4Addr)
{
    NS_LOG_FUNCTION(this);
    Ptr<TcpOptionAddAddr> option = CreateObject<TcpOptionAddAddr>();
    option->SetAddrID(addrID);
    option->SetIpv4Addr(ipv4Addr);

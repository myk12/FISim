...
NS_LOG_COMPONENT_DEFINE ("RttEstimator2");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (RttEstimator2);

TypeId 
RttEstimator2::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RttEstimator2")
    .SetParent<Object> ()
    .AddAttribute ("MaxMultiplier", 
                   "Maximum RTO Multiplier",
                   UintegerValue (64),
                   MakeUintegerAccessor (&RttEstimator2::m_maxMultiplier),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("InitialEstimation", 
                   "Initial RTT estimation",
                   TimeValue (Seconds (1.0)),
                   MakeTimeAccessor (&RttEstimator2::m_initialEstimatedRtt),
                   MakeTimeChecker ())
    .AddAttribute ("MinRTO", 
                   ...
                   MakeTimeAccessor (&RttEstimator2::SetMinRto,
                                     &RttEstimator2::GetMinRto),
                   MakeTimeChecker ())
  ;
  return tid;
}
...
void 
RttEstimator2::SetMinRto (Time minRto)
{
  NS_LOG_FUNCTION (this << minRto);
  m_minRto = minRto;
}
Time 
RttEstimator2::GetMinRto (void) const
{
  return m_minRto;
}
void 
RttEstimator2::SetCurrentEstimate (Time estimate)
{
  NS_LOG_FUNCTION (this << estimate);
  m_currentEstimatedRtt = estimate;
}
Time 
RttEstimator2::GetCurrentEstimate (void) const
{
...
//RttHistory2 methods
RttHistory2::RttHistory2 (SequenceNumber32 s, uint32_t c, Time t)
  : seq (s), count (c), time (t), retx (false)
{
  NS_LOG_FUNCTION (this);
}

RttHistory2::RttHistory2 (const RttHistory2& h)
...
RttEstimator2::RttEstimator2 (const RttEstimator2& c)
...
TypeId
RttEstimator2::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

void RttEstimator2::SentSeq (SequenceNumber32 seq, uint32_t size)
{ 
  NS_LOG_FUNCTION (this << seq << size);
  // Note that a particular sequence has been sent
  if (seq == m_next)
    { // This is the next expected one, just log at end
      m_history.push_back (RttHistory2 (seq, size, Simulator::Now () ));
      m_next = seq + SequenceNumber32 (size); // Update next expected
    }
  else
    { // This is a retransmit, find in list and mark as re-tx
      for (RttHistory2_t::iterator i = m_history.begin (); i != m_history.end (); ++i)
        {
...
Time RttEstimator2::AckSeq (SequenceNumber32 ackSeq)
{ 
...
void RttEstimator2::IncreaseMultiplier ()
{
  NS_LOG_FUNCTION (this);
...
void RttEstimator2::ResetMultiplier ()
{
  NS_LOG_FUNCTION (this);
  m_multiplier = 1;
}

void RttEstimator2::Reset ()
{ 
  NS_LOG_FUNCTION (this);
...
NS_OBJECT_ENSURE_REGISTERED (RttMeanDeviation2)
  ;

TypeId 
RttMeanDeviation2::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RttMeanDeviation2")
    .SetParent<RttEstimator2> ()
    .AddConstructor<RttMeanDeviation2> ()
    .AddAttribute ("Gain",
                   "Gain used in estimating the RTT, must be 0 < Gain < 1",
                   DoubleValue (0.1),
                   MakeDoubleAccessor (&RttMeanDeviation2::m_gain),
                   MakeDoubleChecker<double> ())
  ;
  return tid;
}

RttMeanDeviation2::RttMeanDeviation2() :
  m_variance (0) 
{ 
  NS_LOG_FUNCTION (this);
}

RttMeanDeviation2::RttMeanDeviation2 (const RttMeanDeviation2& c)
  : RttEstimator2 (c), m_gain (c.m_gain), m_variance (c.m_variance)
{
  NS_LOG_FUNCTION (this);
}

TypeId
RttMeanDeviation2::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

void RttMeanDeviation2::Measurement (Time m)
{
...
Time RttMeanDeviation2::RetransmitTimeout ()
{
  NS_LOG_FUNCTION (this);
...
Ptr<RttEstimator2> RttMeanDeviation2::Copy () const
{
  NS_LOG_FUNCTION (this);
  return CopyObject<RttMeanDeviation2> (this);
}

void RttMeanDeviation2::Reset ()
{ 
  NS_LOG_FUNCTION (this);
  // Reset to initial state
...
void RttMeanDeviation2::Gain (double g)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG( (g > 0) && (g < 1), "RttMeanDeviation2: Gain must be less than 1 and greater than 0" );
  m_gain = g;
}
